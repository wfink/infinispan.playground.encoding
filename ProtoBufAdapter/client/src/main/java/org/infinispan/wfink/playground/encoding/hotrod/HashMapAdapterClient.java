...
/**
 * A simple client using the HashMapAdapter to store a HashMap directly to a cache. Infinispan 12+ is mandatory.</br>
 * An adapter is needed because the current Infinispan 14 does not support HashMap by default, this is tracked by <a href="https://issues.redhat.com/browse/ISPN-14438">ISPN-14438</a>
 *
 * @author <a href="mailto:WolfDieter.Fink@gmail.com">Wolf-Dieter Fink</a>
 */
public class HashMapAdapterClient {
  private RemoteCacheManager remoteCacheManager;
  private RemoteCache<String, HashMap<String, String>> mapCache;

  public HashMapAdapterClient(String host, String port, String cacheName) {
...
//    registerSchemas(remoteCacheManager, initializer);  // not needed as long as the example domain classes are copied to the ISPN_HOME/server/lib directory

    mapCache = remoteCacheManager.getCache(cacheName);
  ...
  private void putHashMap() {
    ...
    System.out.printf("added a Map for key %s : %s\n", key, mapCache.get(key));
  }

  private void getAll() {
    System.out.printf("read all %d entries...\n", mapCache.size());
    for (Map.Entry<String, HashMap<String, String>> m : mapCache.entrySet()) {
  ...
  public static void main(String[] args) {
    ...
    HashMapAdapterClient client = new HashMapAdapterClient(host, port, cacheName);
